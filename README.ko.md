...
SourceLedger는 사전에 설정한 상품·출처의 가격을 수집하고 원문·근거를 보존하며 XLSX 보고서를 만드는 로컬 도구입니다. 가격을 추정하거나 빈 값을 0으로 채우지 않습니다.
...
`discover`는 설정된 한 출처를 한 번 읽어 HTML 링크 또는 XML sitemap의 URL 후보를 최대 지정 개수만 반환합니다. 후보는 같은 `allowed_domains` 안의 HTTP(S) URL로 제한되고, fragment·중복·인증 정보 URL은 버립니다. 가격 근거나 자동 추가 설정이 아닙니다.

```powershell
source-ledger discover --config examples/demo.json --source-id catalog --limit 100
```

후보의 상품 범위와 선택자/열 매핑을 검토한 뒤 사람이 설정의 `sources`에 추가해야 합니다. 임의 웹 검색, 자동 학습, 자동 수집 범위 확장은 제공하지 않습니다.
...
- 첫 실행 설정 마법사와 자율 에이전트 반복 구조는 아직 구현하지 않았습니다. 수집 전 상품과 출처를 JSON으로 설정해야 합니다.
- URL 후보 발견은 설정된 HTML/sitemap 출처 안에서만 지원합니다. 임의 웹 검색, 자동 학습, 로그인 자동화는 제공하지 않습니다.
...
스캔 문서 OCR, 설정 출처 밖으로 검색 범위를 넓히는 요구, ERP 연동, 첫 실행 자율 흐름은 관련 출처와 접근 제약이 준비된 뒤 별도 설계·검증이 필요합니다.
