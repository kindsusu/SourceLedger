...
연구 워크스페이스는 상품 주제 하나를, 수집 설정은 여러 개의 정확한 상품을 지원합니다. 후보 발견만으로 추출 규칙을 생성하지는 않으므로 초안의 규칙을 작성·확인해야 합니다.
...
- 첫 실행 설정·후보 저장·초안·설정 검증은 구현했습니다. 검색 공급자 연결, AI 추출 규칙 생성, 자율 에이전트 반복 구조는 남아 있습니다.
...
스캔 문서 OCR, 설정 출처 밖 검색, 자율 규칙 생성은 대표 출처가 준비된 뒤 추가 설계·검증이 필요합니다. ERP 연동과 분석·모의계산 기능은 이번 범위에서 제외했습니다.
