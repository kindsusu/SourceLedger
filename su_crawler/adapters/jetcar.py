...
def _table(soup: BeautifulSoup, *needles: str) -> Tag | None:
    return next((table for table in soup.select("table") if is_visible(table) and all(n in (text(table) or "") for n in needles)), None)
...
def extract(result: FetchResult, source: Source) -> list[Candidate]:
    ...
    if text(option_table):
        option_parts.append(text(option_table) or "")
    ...
    tax_node = next((n for n in soup.select(".cap") if is_visible(n) and "부가세" in (text(n) or "")), None)
    if tax_node:
        fields["tax"] = text(tax_node)
        proofs["tax"] = evidence("css:.cap", fields["tax"], text(tax_node))
    ...
    for index, row in enumerate(rent_table.select("tr") if rent_table else [], start=1):
        ...
        rows.append(Candidate(fields=row_fields, evidence=row_proofs, locator=f"jetcar:{item_id}:rent-row:{index}", extraction_method="jetcar_visible_html", value_origin="observed", source_visibility=visibility))
    if rows:
        return rows
    proofs["price"] = evidence("table:rent", None, text(rent_table) or "No displayed monthly price row")
    return [Candidate(fields=fields, evidence=proofs, locator=f"jetcar:{item_id}", extraction_method="jetcar_visible_html_no_price", value_origin="observed", source_visibility=visibility, review_flags=["missing_displayed_price"])]
