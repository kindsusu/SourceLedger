...
def extract(result: FetchResult, source: Source) -> list[Candidate]:
    ...
    if not all((car, dep, term, amount_node)) or not all(is_visible(node) for node in (car, dep, term, amount_node)):
        return []
    amount_raw = visible_text(amount_node)
    cap_node = soup.select_one("#estOut .cap")
    cap_raw = visible_text(cap_node)
    basis_text = f"{cap_raw or ''} {amount_raw or ''}"
    ...
    car_raw, dep_raw_selected, term_raw = visible_text(car), visible_text(dep), visible_text(term)
    fields = {"item_id": item_id, "name": car_raw, "currency": "KRW", "price_basis": "monthly", "term_months": decimal_string(str(term_id))}
    proofs = {
        "item_id": evidence("css:#estCar/#estDep/#estTerm selected options", item_id, f"{car_raw} | {dep_raw_selected} | {term_raw}"),
        "name": evidence("css:#estCar option[selected]", fields["name"], car_raw),
        ...
        "term_months": evidence("css:#estTerm option[selected]", fields["term_months"], term_raw),
    }
    dep_percent = re.search(r"(\d+(?:\.\d+)?)\s*%", dep_raw_selected or "")
    if dep_percent:
        fields["deposit_percent"] = dep_percent.group(1)
        proofs["deposit_percent"] = evidence("css:#estDep option[selected]", fields["deposit_percent"], dep_raw_selected)
    dep_line = soup.select_one("#estDepLine")
    dep_raw = visible_text(dep_line)
    ...
    return [Candidate(fields=fields, evidence=proofs, locator=f"funrent:{item_id}", extraction_method="funrent_rendered_calculator", value_origin="calculator_estimate", source_visibility=visibility, derived_values=derived, review_flags=flags, evidence_mode=mode)]
